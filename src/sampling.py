...
def pushforward_generator_fn(
    ...
    if discretization == "uniform":
        t_steps = torch.linspace(
            net.T, net.eps, num_steps + 1, dtype=torch.float64, device=latents.device
        )
    elif discretization == "edm":
        nt_min = (
            net.get_log_nt(torch.as_tensor(net.eps, dtype=torch.float64)).exp().item()
        )
        nt_max = (
            net.get_log_nt(torch.as_tensor(net.T, dtype=torch.float64)).exp().item()
        )
        rho = 7
        step_indices = torch.arange(
            num_steps + 1, dtype=torch.float64, device=latents.device
        )
        nt_steps = (
            nt_max ** (1 / rho)
            + step_indices / (num_steps) * (nt_min ** (1 / rho) - nt_max ** (1 / rho))
        ) ** rho
