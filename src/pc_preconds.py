...
class pc_IMMPrecond(torch.nn.Module):
    def __init__(
        ...
        **model_kwargs,  # Keyword arguments for the underlying model.
        ...
        self.nt_low = (
            self.get_log_nt(torch.tensor(self.eps, dtype=torch.float64))
            .exp()
            .numpy()
            .item()
        )
        self.nt_high = (
            self.get_log_nt(torch.tensor(self.T, dtype=torch.float64))
            .exp()
            .numpy()
            .item()
        )
    ...
    def euler_fm_sample_function(self, yt, y, t, s):
        assert self.noise_schedule == "fm"
        assert yt.shape == y.shape, "Dimension mismatch"
        return (
            yt - self.sigma_data * (t - s).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * y
        )
        ...
        **model_kwargs,
        ...
        if cfg_scale is not None:
            ...
            class_labels = torch.cat(
                [torch.zeros_like(class_labels), class_labels], dim=0
            )
        else:
            x_cfg = x
        F_x = self.forward_model(
